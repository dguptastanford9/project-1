...
def runOpenAi(env_name, num_bins, alpha, epsilon, gamma, alpha_decay, epsilon_decay, numTraining, display_graphics, agent_class, discretize, sim_env):
    env = gym.make(env_name)
    recording_str = 'recording/'+env_name+'-'+time.strftime('%H%M%S')
    env = wrappers.Monitor(env, recording_str)
    openai_learner = OpenAiLearner(env, env_name, num_bins, alpha, epsilon, gamma, alpha_decay, epsilon_decay, numTraining, agent_class, discretize, sim_env)
...
class OpenAiLearner:
    ...
    def __init__(self, env, env_name, num_bins, alpha, epsilon, gamma, alpha_decay, epsilon_decay, numTraining, agent_class, discretize, sim_env):
        self.env = env
...
def read_command(argv):
    ...
    parser.add_option('--learningDecay', type='float', dest='alpha_decay', help=default('Learning rate decay factor'),
                        default='1.0')
    parser.add_option('--explorationDecay', type='float', dest='epsilon_decay', help=default('Exploration rate decay factor'),
                        default='1.0')
    parser.add_option('-x', '--numTraining', type='int', dest='numTraining', help=default('Number of training episodes'),
                        default='1500')
